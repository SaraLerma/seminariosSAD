...
exports.addPrivateMessage = function (msg){
	...
	var privatePost = new Post(msg.msg, u1, msg.ts);
	privateMessages[k].push ();
	console.log(privateMessages);
	return { user: k, message: privatePost }
}

exports.getPrivateMessageList = function (u1, u2) {
	// lower user us always first
	var k = getUserKey(u1, u2);
	console.log ("private message list key:" + k);
	return JSON.stringify (privateMessages[k]);
}

exports.getUserKey = function (u1, u2) {
	return (u1.toLowerCase().localeCompare(u2.toLowerCase())<0) ? u1+'_'+u2:u2+'_'+u1;
}
...
exports.addPublicMessage = function (msg)
{
	...
	for(var idx in subjects) {
		if(subjects[idx] == msg.to || idx==msg.to) {
			var ml = publicMessages [idx];
			console.log("indice es :" + idx);
			if (!ml) publicMessages [idx] = [];
			publicPost = new Post(msg.msg, msg.from, msg.ts);
			publicMessages[idx].push();
			msg.to=idx;
			console.log(publicMessages);
		}
	}
	return { subject: msg.to, message: publicPost}
}

exports.getPublicMessageList = function (sbj) {
	for(var idx in subjects) {
		if(subjects[idx] == sbj || idx==sbj) {
			sbj=idx;
			return JSON.stringify (publicMessages[idx]);
		}
	}
	return undefined;
}
exports.getIdOrNameSubject = function(sbj){
	for(var idx in subjects) {
		if(subjects[idx] == sbj){
			return idx;
		} else if(idx==sbj){
